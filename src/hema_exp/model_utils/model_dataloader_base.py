# 2024.12.18 This is base dataloader
# The dataset and dataloader need to be further added
import os
import pickle
import torch
import pandas as pd
...
from hema_exp.model_utils.model_sampler import WeightedKeepLabelSampler


# copy from Hematoma2dDataset
# The dataset and dataloader need to be further added
# It was disassembled from the original "Hematoma2dDataset" in order to remove the strong dependency on "tio"
class ImageBaseDataset:
    def __init__(self, config, logger=None):
        ...
        # 2024.12.11, debug, Using []] instead of "None" to prevent errors in the for loop when the dictionary is empty
        self.train_trans_dic = self.cf['transformer'].get('train', [])
        self.valid_trans_dic = self.cf['transformer'].get('valid', [])
        self.predict_trans_dic = self.cf['transformer'].get('predict', [])

    def load_data_by_dir(self, img_dir, mask_dir=None, label_file_path=None, 
                         img_name_in_label_file_head = "name", label_in_label_file_head = "label",
        ...
        :param label_file_path: string, path of a csv file. This file stroing all image name and lables.
                                If mode is 'train' or 'valid', this arg is required
        :param img_name_in_label_file_head: string, colname of img name in label file
        :param label_in_label_file_head: string, colname of label in label file
        ...
        if label_file_path:
            ...
            img_label_dic = {i[0]: i[1] for i in zip(
                label_df[img_name_in_label_file_head], label_df[label_in_label_file_head])}
        ...
        for n, i in enumerate(match_dic.keys()):
            ...
            if label_file_path:
                match_dic_cp[i]["label"] = img_label_dic[os.path.basename(img_path[0])]
    ...
    def load_data_by_cv_fold_file(self, fold_file_path, valid_fold_n, 
                                  ignore_fold_n = None, is_only_setting_one_mode=False,
        ...
        # convert ignore_fold_n to list
        if isinstance(ignore_fold_n, int):
            ignore_fold_n = [ignore_fold_n]
        elif not ignore_fold_n:
            ignore_fold_n = []
            
        if self.logger:
            self._set_logging("info", "valid fold: {}, ignore fold: {}".format(valid_fold_n, ignore_fold_n))
        ...
        if is_only_setting_one_mode:
            ...
            if dataset_mode == 'train':
                train_dic = {k: v for k, v in fold_path_dic.items() if k not in (list(valid_fold_n) + list(ignore_fold_n))}
                self.train_dic = reduce(lambda x, y: {**x, **y}, train_dic.values())  # merge all train dic
            elif dataset_mode == 'valid':
                valid_dic = {k: v for k, v in fold_path_dic.items() if k in list(valid_fold_n)}
                self.valid_dic = reduce(lambda x, y: {**x, **y}, valid_dic.values())  # merge all valid dic
            elif dataset_mode == 'predict':
                pred_dic = {k: v for k, v in fold_path_dic.items() if k in list(valid_fold_n)}
    ...
    def get_dataset(self, dataset_mode='train'):
        ...
        :return: dataset, which format to be further confirmed
        """
        # get data dic
    ...
    # 2025.03.03, add sampler_name
    def get_sampler(self, weight_dic, dataset_mode='train', 
                    sampler_name="WeightedRandomSampler", **kwargs):
        """
        ...
        if sampler_name == "WeightedRandomSampler":
            ...
            keep_labels = kwargs["keep_labels"]
            sampler = WeightedKeepLabelSampler(label_weights_dict=weight_dic, keep_label_list=keep_labels,
                                               label_list=label_list, num_samples=len(label_list))
        else:
            self._set_logging('error', f'Unsupport sampler name: {sampler_name}!')
